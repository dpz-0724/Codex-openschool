...
- 已新增新手首读入口：[从这里开始：AI Agent 新手导读](START_HERE.md)，按时间、目标和风险水平给读者分流。
- 已新增低风险练习入口：[Agent 新手低风险练习室](labs/README.md)，把路线拆成可直接执行的小任务。
- 已新增一手资源入口：[AI Agent 一手资源中文导读](resources/README.md)，把官方资料转成阅读顺序、误读提醒和落地练习。
- 已补齐 17 篇高质量入门笔记；新增内容必须先判断是否能解决一个具体问题，避免为了数量扩张。
...
- 已从 AIHot 最新条目中补充 4 个通过核验的公开 skills / skill 项目：[xiaohu-wechat-format](https://github.com/xiaohuailabs/xiaohu-wechat-format)、[warpdotdev/common-skills](https://github.com/warpdotdev/common-skills)、[text-to-lottie](https://github.com/diffusionstudio/lottie)、[baoyu-design](https://github.com/JimLiu/baoyu-design)。
...
- 已把 AIHot 里的 `qiaomu-goal-meta-skill` 拆成常青教程和可复制提示词：[Codex Goal 指令怎么写](notes/codex-goal-writing-template.md)。
...
- 已新增贡献规范和 GitHub issue 模板：[CONTRIBUTING.md](CONTRIBUTING.md)，先用脱敏门禁接收高质量反馈。

## 50-150 Stars 的重点任务

- 把最受欢迎的 3 篇内容扩展成系列。
- 每篇教程配一张可复制的任务清单或提示词。
- 持续接收 issue 反馈，但不接受未脱敏材料、泛泛工具链接或没有正文的选题。
- 持续维护外部优秀资源中文导读专题，优先补官方资料、开源仓库和可落地练习。
