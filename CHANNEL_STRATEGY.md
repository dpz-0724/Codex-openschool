...
- 增加 3-5 个可复制实战任务。
- 用贡献规范和 issue 模板接收选题，但先筛隐私、来源和新手价值。
...
收到 issue 或 PR 后，先按这个顺序处理：

1. 判断是否包含账号、聊天记录、客户材料、密钥、本地路径、后台截图或未脱敏内容；有则先要求脱敏，不进入正文讨论。
2. 判断是否解决新手真实问题；只有工具名、标题或宣传语的建议进入观察池，不进主目录。
3. 判断是否有公开来源；没有来源的内容只能作为“本地整理”候选，不能写成外部事实。
4. 判断是否能写成正文、流程、表格、模板或实战任务；不能成文的只保留为待办。
5. 合并前检查 README、目录、内部链接、隐私扫描和传播素材是否需要同步更新。
