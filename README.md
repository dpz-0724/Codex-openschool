...
- 学习笔记：审计后只保留已经整理成公开 MD 的高价值笔记，优先服务 Agent 入门、术语表、概念辨析、第一任务、工具调用、浏览器调试、权限分级、代码审查、skills 触发边界、Spec 模板、AI 可执行 PRD、Goal 指令、长任务环境规格、实战和避坑。
...
- 数量：18 篇已成文、已脱敏、可直接阅读的公开笔记。
- 入口：[notes/README.md](notes/README.md)
- 内容：Agent 新手路线、新手术语表、核心概念 FAQ、第一个 Agent 实战任务、自动化浏览、Codex 浏览器开发者模式、MCP vs CLI 工具调用选型、Agent 自主权限分级、Agent 代码审查工作流、Skill 触发边界、Agent 编程前 Spec 模板、AI 可执行 PRD、Codex Goal 指令、Agent 长任务环境规格、Enterprise Agent Harness 与环境工程、企业数据治理、PPT Master 工作流、个人知识库搭建。
